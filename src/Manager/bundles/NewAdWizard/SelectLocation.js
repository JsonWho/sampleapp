...
class SelectLocation extends React.Component {
	...
	componentDidMount() {
		...
		this.props.setStepData('select_location',3);
	}
...
render() {
			...
			const list = this.props.data.map(l => 
				<div key={l.id} onClick={() => this.optionClicked(l)} className={'row' + this.isOptionSelected(l.id)}><div>{l.name}</div><div>{l.streetNo+' '+l.street}</div><div>{l.suburb}</div></div> )
